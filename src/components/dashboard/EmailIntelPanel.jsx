  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-border shrink-0">
        ...
        {activeTab === 'intel' && data?.scanned_at && !isLoading && (
          <span className="text-[10px] text-muted-foreground">
            {formatDistanceToNow(new Date(data.scanned_at), { addSuffix: true })}
          </span>
        )}
        {activeTab === 'intel' && (
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={scan} disabled={isLoading} title="Scan emails now">
      ...
      <div className="flex border-b border-border shrink-0">
        <button
          ...
          className={`flex-1 py-2 text-xs font-medium transition-colors ${activeTab === 'intel' ? 'text-foreground border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'}`}
        >
          Overview
        </button>
        <button
          onClick={() => setActiveTab('search')}
          className={`flex-1 py-2 text-xs font-medium transition-colors ${activeTab === 'search' ? 'text-foreground border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'}`}
        >
          Search
        </button>
      </div>
